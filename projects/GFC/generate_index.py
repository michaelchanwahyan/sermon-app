...
_ = os.system("wget https://www.youtube.com/@graceflowchurch6874/videos")
'''
...
ls *.mp3 > ~/SOURCE/sermon-app/projects/GFC/exlist.txt

#vim ~/SOURCE/sermon-app/projects/GFC/exlist.txt # edit to only preserve the 11-character hash code'''
...
print('Grace Flow Church completed list contains %d' % len(_list))
'''
...
'''
cd ~/TPPHC/SERMON/GFC/
ls *.mp3 > ~/SOURCE/sermon-app/projects/GFC/ls.txt
'''
...
rdd = sc.textFile('ls.txt') \
    ...
    .map(lambda w: w.replace('  ', ' ')) \
    .map(lambda w: w.replace('  ', ' ')) \
    .map(lambda w: w.replace(' 講道 ', '')) \
    .map(lambda w: w.replace('講員：', '')) \
    .map(lambda w: w.replace('（', '（ ')) \
    .map(lambda w: w.replace('）', '） '))
