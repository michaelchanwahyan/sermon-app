...
import os
_ = os.system("rm -f cgst.videos")
_ = os.system("wget -O cgst.videos https://www.youtube.com/@cgstedu/videos")
_ = os.system("rm -f cgst.streams")
_ = os.system("wget -O cgst.streams https://www.youtube.com/@cgstedu/streams")
_ = os.system("cat cgst.videos cgst.streams > videos")
_ = os.system("rm -f cgst.*")
...
cd ~/TPPHC/SERMON/CGST/

ls *.mp3 > ~/SOURCE/sermon-app/projects/CGST/exlist.txt

vim ~/SOURCE/sermon-app/projects/CGST/exlist.txt # edit to only preserve the 11-character hash code'''
...
print('China Graduate School of Theology completed list contains %d' % len(_list))
...
preacherTitle_list = ['博士','牧師','傳道','老師','先生','教授','弟兄','社長', '神學生', '師母', '院長']
...
ls -logtD '%b %d  %Y' *.mp3 | awk '{print substr($0,index($0,$4))}' > ~/SOURCE/sermon-app/projects/CGST/lslogt.txt
'''
...
rdd2 = rdd1.map(
    lambda w: ([j for i in preacherTitle_list \
               ...
               w[1], w[-2], w[-1])) \
    .map(lambda w: ([ _.replace('主講', '') \
                       .replace('榮休院長', '') \
                       .replace('見證分享', '') \
                       .replace('回應', '') for _ in w[0] ], w[1], w[2], w[3], w[4]))
