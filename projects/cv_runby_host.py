...
foldername = "JNG"
mp3filelist = os.listdir(f"/Users/pikachu/OneDrive - The Chinese University of Hong Kong/TPPHC/SERMON/{foldername}/")
codes = list(set([ mp3file[-15:-4] for mp3file in mp3filelist ]))
codes = [ code for code in codes \
        if code[0]!="-" \
            and not os.path.isfile(f"/Users/pikachu/SOURCE/sermon-app/data/{foldername}/{code}.txt") ]
print(f"total no. of codes: {len(codes)}")
for code in codes:
    cmdstr = f"ffmpeg -i ~/One*/TPPHC/SERMON/{foldername}/*{code}.mp3 ~/SOURCE/sermon-app/projects/{code}.wav"
