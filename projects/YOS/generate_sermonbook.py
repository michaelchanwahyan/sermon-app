...
def print_prefix(sermon_tex_filepath, progressStepCnt):
    ...
    _ = os.system(f"cat ../prefix.tex | sed 's/粵語講道逐字稿/友愛堂信培部 粵語主日學逐字稿/' | sed 's/Youtube Channel:/Youtube Channel: Yau Oi School/' > " + sermon_tex_filepath)
...
def generate_toc(sermon_tex_filepath, index_file, toc_type, progressStepCnt):
    ...
    with open(sermon_tex_filepath, "a") as fp:
        ...
        fp.write("\\hline\n")
        # --------------------------------------
        # lines is the line content in toc_type
        # --------------------------------------
        for lineId, line in enumerate(lines):
            ...
            # --------------------------------------
            if os.path.isfile(f'../../data/YOS/{cc}.txt'):
...
def write_sermon_text(fp, cc):
    with open("../../data/YOS/"+cc+".txt", "r") as fp_:
...
def write_sermon_section(sermon_tex_filepath, cc, cc_prev, cc_next):
    with open(sermon_tex_filepath, "a") as fp:
        ...
        fp.write("~\n")
        fp.write("\\hyperref[sec:index_preacher]{\\small{[返主目錄]}}\n")
        fp.write("~\n")
...
def generate_main_content(sermon_tex_filepath, index_file, toc_type, progressStepCnt):
    with open(index_file, "r") as fp:
        lines = fp.readlines()
    progressStepCnt += 1
...
def sermon_tex():
    ...
    progressStepCnt = 0
    sermon_tex_filepath = f"../../build/YOS/sermon_YOS.tex"
    # --------------------------------------
    ...
    # --------------------------------------
    progressStepCnt = generate_toc(sermon_tex_filepath, './index_byp.csv', 'preacher', progressStepCnt)
    progressStepCnt = generate_main_content(sermon_tex_filepath, './index_byp.csv', 'preacher', progressStepCnt)
    # --------------------------------------
