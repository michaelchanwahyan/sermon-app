...
ls *.mp3 > ~/SOURCE/sermon-app/projects/KFC/exlist.txt
...
rgx_kfc_dateterm = recompile(r'([0-9]*)月([0-9-]*)日')
...
rdd_chi = rdd.filter(lambda w: '崇拜' in w) \
    .map(lambda w: rgx_kfc_dateterm.sub('\\1月\\2日 ', w))


for _ in rdd_chi.take(3):
    print(_)


rdd_eng = rdd.filter(lambda w: 'Worship Service' in w)


for _ in rdd_eng.take(3):
    print(_)
...
rdd1_checkcode_1tomany = rdd_chi1.map(lambda w: (w[1], 1)).reduceByKey(add)
...
rdd1_checkcode_1tomany = rdd_eng1.map(lambda w: (w[1], 1)).reduceByKey(add)
...
rdd_chi2 = rdd_chi1.map(
    lambda w: ([j for i in preacherTitle_list \
               ...
               w[1], w[-2], w[-1])) \
    .map(lambda w: ([ re.sub(r'[0-9]', '', _) for _ in w[0] ], w[1], w[2], w[3], w[4]))


# data engineering work 2: preacher stop word
rdd_eng2 = rdd_eng1.map(
    lambda w: ([],
               list(set([i for i in book_list for j in w[0] if i in j])),
               w[1], w[-2], w[-1])) \
    .map(lambda w: ([ re.sub(r'[0-9]', '', _) for _ in w[0] ], w[1], w[2], w[3], w[4]))
...
for _ in rdd_chi2.take(40):
    print(_)


print('w[0]= preacher ; w[1]= book; w[2]= youtube code ; w[3]= original name ; w[4]= date')
for _ in rdd_eng2.take(40):
    print(_)


# data engineering work 3: removing the initials of preacher
preacher_list = rdd_chi2.map(lambda w: w[0]) \
...
for (c, s) in rdd_chi2.map(lambda w: (w[2], w[3])).collect():
    c2s_dict[c] = s.strip()

for (c, s) in rdd_eng2.map(lambda w: (w[2], w[3])).collect():
    c2s_dict[c] = s.strip()
...
for (c, p) in rdd_chi2.map(lambda w: (w[2], w[0])).collect():
    if p is None:
        continue
    if len(p) == 0:
        continue
...
for (c, p) in rdd_eng2.map(lambda w: (w[2], w[0])).collect():
    if p is None:
        continue
    if len(p) == 0:
        continue
...
for _ in rdd_chi2.map(lambda w: (w[2], w[1])).take(3):
    print(_)


c2b_dict = {} # 1-to-1 dictionary
b2c_dict = {} # 1-to-N dictionary
for (c, b) in rdd_chi2.map(lambda w: (w[2], w[1])) \
...
for (c, b) in rdd_eng2.map(lambda w: (w[2], w[1])) \
                  .filter(lambda w: len(w[1]) > 0) \
                  .map(lambda w: (w[0], ''.join(w[1]))).collect():
    # 1-to-1 dictionary
    c2b_dict[c] = b
...
for (c, t) in rdd_chi2.map(lambda w: (w[2], w[-1])).collect():
    c2t_dict[c] = t

for (c, t) in rdd_eng2.map(lambda w: (w[2], w[-1])).collect():
    c2t_dict[c] = t
