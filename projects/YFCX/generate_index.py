...
import os
_ = os.system("rm -f yfcy.streams")
_ = os.system("wget -O yfcy.streams https://www.youtube.com/YanfookYouth/streams")
_ = os.system("rm -f yfc.streams")
_ = os.system("wget -O yfc.streams https://www.youtube.com/yanfookchurch/streams")
_ = os.system("cat yfcy.streams yfc.streams > videos")
_ = os.system("rm -f yfc*.*")
...
cd ~/TPPHC/SERMON/YFCX/

ls > ~/SOURCE/sermon-app/projects/YFCX/exlist.txt

vim ~/SOURCE/sermon-app/projects/YFCX/exlist.txt # edit to only preserve the 11-character hash code'''
...
print('Yan Fook completed list contains %d' % len(_list))
...
rdd1 = rdd.map(lambda w: w.replace('｜', '｜').replace('︱', '｜').replace('|','｜'))\
    .map(lambda w: (w[:-18].strip(), w[-16:-5])) \
