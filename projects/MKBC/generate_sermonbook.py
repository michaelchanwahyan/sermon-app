...
def mkbc_title_processing(cc):
    sstr = c2s_dict.get(cc, ' ')
    sstr = re.sub(r'[0-9][0-9][0-9][0-9]年[0-9]*月[0-9]*日', '', sstr) # remove date from title
    sstr = rgx_bv.sub(':', sstr) # adjust bible verse , use ':' to replace '_'
    sstr = sstr[:-13] # remove trailing youtube code
    sstr = sstr.replace('主餐暨堂慶主日崇拜直播', '')
    sstr = sstr.replace('青少主日崇拜直播', '')
    sstr = sstr.replace('長者主日崇拜直播', '')
    sstr = sstr.replace('早堂崇拜直播', '')
    sstr = sstr.replace('主餐崇拜直播', '')
    sstr = sstr.replace('崇拜直播', '')
...
def print_prefix(sermon_tex_filepath, progressStepCnt):
    ...
    _ = os.system(f"cat ../prefix.tex | sed 's/粵語講道逐字稿/旺角浸信會 粵語講道逐字稿 2020-present/' | sed 's/Youtube Channel:/Youtube Channel: MongKok Baptist Church/' > " + sermon_tex_filepath)
...
def generate_toc(sermon_tex_filepath, index_file, toc_type, progressStepCnt):
    ...
    with open(sermon_tex_filepath, "a") as fp:
        ...
        for lineId, line in enumerate(lines):
            ...
            # --------------------------------------
            if os.path.isfile(f'../../data/MKBC/{cc}.txt'):
                bstr = c2b_dict.get(cc, ' ')
                vstr = c2v_dict.get(cc, ' ')
                sstr = c2s_dict.get(cc, ' ')
                sstr = mkbc_title_processing(cc)
                sstr = cleanse_special_char(
...
def write_sermon_text(fp, cc):
    with open(f"../../data/MKBC/{cc}.txt", "r") as fp_:
...
def sermon_tex():
    progressStepCnt = 0
    sermon_tex_filepath = f"../../build/MKBC/sermon_MKBC_2020-present.tex"
    # --------------------------------------
