...
FN_WWBC=$(   ls -1 ./data/WWBC    | wc -l)
FN_YFCX=$(   ls -1 ./data/YFCX    | wc -l)
FN_TOTAL=$(echo $FN_CBI + $FN_CGST + $FN_DSCCC + $FN_FVC + $FN_HKBC + $FN_JNG + $FN_WWBC + $FN_YFCX | bc)
...
echo sermon count in WWBC    : $FN_WWBC    / $FN_TOTAL \( $(printf "%.1f" $((10**4 * FN_WWBC    / FN_TOTAL))e-2)% \)
...
CHNG_CNT_WWBC=$(   git --no-pager show --name-only --oneline $OU | grep WWBC    | wc -l) # for each hash count the file updates in specified sermon source
...
echo activeness on WWBC    : $CHNG_CNT_WWBC    / $CHNG_CNT_TOTAL \( $(printf "%.1f" $((10**4 * CHNG_CNT_WWBC    / CHNG_CNT_TOTAL))e-2)% \)
...
echo WWBC    \| $(printf "%.1f" $((10**4 * FN_WWBC  / FN_TOTAL))e-2)% \| $(printf "%.1f" $((10**4 * CHNG_CNT_WWBC  / CHNG_CNT_TOTAL))e-2)%
